<template>
  <div class="mb-6">
    <div class="flex flex-col sm:flex-row gap-4">
      ...
      <div class="flex-1">
        <div class="relative">
          <input
            v-model="searchInput"
            aria-label="Search player by name or SteamID"
    ...
    <!-- Error Message -->
    <div v-if="error" class="mt-4 p-4 rounded-lg border" style="background-color: #7f1d1d; border-color: #dc2626;" role="alert">
    ...
    <div v-if="searchResults.length > 0" class="mt-4" aria-live="polite">
      ...
      <div class="results-list">
        ...
        >
          ...
          <span class="result-points">{{ Number(p.points || 0).toLocaleString('en-GB') }} pts</span>
...
const searchPlayer = async () => {
  ...
  try {
    ...
    if (data.success) {
      ...
      error.value = data?.error?.message || data.error || 'Player search failed.';
    }
  } catch (err) {
