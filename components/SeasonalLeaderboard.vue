  ...
  <div class="leaderboard-container">
    ...
    <div class="search-section">
      ...
      <div v-else-if="!loading && players.length > 0">
        ...
        <div v-if="topThree.length > 0">
          ...
          <div class="podium-grid">
            <div
                ...
                @keydown.enter="openPlayerModal(player.steamid)"
                @keydown.space.prevent="openPlayerModal(player.steamid)"
        ...
        <div v-if="scrollableList.length > 0">
          ...
          <div class="flex justify-center pt-8">
            <p class="footer-text">
              Showing {{ players.length }} players • Last updated: {{ todayLabel }}
            </p>
      ...
      <div v-else class="empty-state">
        ...
        <p v-if="error" class="footer-error" role="alert">{{ error }}</p>
      </div>
...
const todayLabel = computed(() => new Date().toLocaleDateString('en-GB', { timeZone: 'Europe/Berlin' }));
...
const fetchLeaderboard = async () => {
  ...
  try {
    ...
    if (data.success) {
      ...
      error.value = data?.error?.message || data.error || 'Leaderboard data could not be loaded.';
      players.value = data.data?.players || [];
    }
  } catch (err) {
...
const formatStatValue = (value, statType) => {
  ...
  switch (statType) {
    case 'topspeed':
      return `${numericValue.toLocaleString('en-GB')} mph`;
    case 'playtime':
      return `${Math.round(numericValue / 3600)} hrs`;
    case 'points':
