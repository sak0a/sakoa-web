<template>
  <div>
    ...
    <!-- Error State -->
    <div v-else-if="error && servers.length === 0" class="glass-card p-8 text-center border border-red-500/20">
      <div class="flex flex-col items-center justify-center space-y-6">
        <div class="relative">
        ...
        <div>
          ...
          <button type="button" class="btn btn-outline btn-sm mt-4" @click="initializeServers">
            Try Again
          </button>
        </div>
      </div>
    ...
    <div v-else class="space-y-6">
      <p v-if="error" class="glass-card p-3 text-sm text-amber-300" role="status">
        Live refresh failed. Showing the most recent available result.
      </p>
      <div v-for="server in servers" :key="server.id" class="relative">
        ...
        <div v-if="server.status === 'online'" class="glass-card p-8 relative border border-green-500/20 hover:border-green-500/40 transition-all duration-300 group">
          ...
          <div class="flex flex-col md:flex-row items-center justify-between mb-6 mt-8">
            ...
            <div>
              <a
                :href="server.comingSoon ? undefined : safeConnectUrl(server)"
                :aria-disabled="server.comingSoon || !safeConnectUrl(server)"
            ...
            <div class="flex items-center mb-4 md:mb-0">
              ...
              <div>
                ...
                <p v-else class="text-gray-400 text-sm">
                  ...
                  <a :href="discordUrl" target="_blank" rel="noopener noreferrer" class="text-blue-400 underline hover:text-blue-300 transition-colors">
                    Join our Discord
                  </a>
                  for updates!
                </p>
...
defineProps({
  discordUrl: {
    type: String,
    default: 'https://discord.gg/JuxYYVEkzc'
  }
});
...
const formatLastChecked = (timestamp) => {
  ...
  if (diffSeconds < 60) {
    ...
    return checked.toLocaleTimeString('en-GB', {
      hour: '2-digit',
      minute: '2-digit',
...
const safeConnectUrl = (server) => {
  const connectUrl = String(server.connectUrl || '');
  return /^steam:\/\/connect\/[a-z0-9.-]+:\d{1,5}$/i.test(connectUrl) ? connectUrl : undefined;
};
</script>
