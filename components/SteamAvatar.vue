<template>
  <div class="steam-avatar-container" :class="containerClass">
    ...
    <div
      v-else
      class="steam-avatar-wrapper"
      :class="{ 'is-clickable': clickable && safeProfileUrl }"
      :style="{ width: size, height: size }"
      :role="clickable && safeProfileUrl ? 'link' : undefined"
      :tabindex="clickable && safeProfileUrl ? 0 : undefined"
      :aria-label="clickable && safeProfileUrl ? `Open ${personaName}'s Steam profile` : undefined"
      @click="onAvatarClick"
...
const safeProfileUrl = computed(() => {
  try {
    ...
    return url.protocol === 'https:' && (hostname === 'steamcommunity.com' || hostname.endsWith('.steamcommunity.com'))
      ? url.toString()
      : null;
  } catch {
    return null;
...
const onAvatarClick = () => {
  if (props.clickable) {
    if (safeProfileUrl.value) {
      window.open(safeProfileUrl.value, '_blank', 'noopener,noreferrer');
    }
...
.steam-avatar-wrapper.is-clickable,
.steam-avatar-wrapper.is-clickable .steam-avatar-image {
  cursor: pointer;
}
